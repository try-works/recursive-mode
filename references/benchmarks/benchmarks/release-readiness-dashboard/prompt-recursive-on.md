...
- Keep the run on `Workflow version: recursive-mode-audit-v2`; do not downgrade the workflow profile.
...
- In Phase 1, build `## Source Requirement Inventory` from `00-requirements.md` so each source obligation is preserved with a source quote before planning.
- In Phase 2, preserve those source obligations losslessly with `## Requirement Mapping`, `## Plan Drift Check`, and Phase 2 `## Requirement Completion Status`.
- Run `python .\\scripts\\lint-recursive-run.py --repo-root . --run-id {{RUN_ID}}` after drafting Phase 2 and again before final closeout; if it fails, repair the current phase and re-audit before locking.
- Do not treat artifact presence alone as success; Phase 2 is not complete until the stricter guardrail sections are present and audit-clean.
